...
public class EncryptWrapper extends HttpServletRequestWrapper {
	/*
	 * HttpServletRequestWrapper - HttpServletRequest 객체를 가공할 수 있는 객체 - 기본 생성자가 존재하지
	 * 않아서 상속된 매개변수 1개짜리 생성자를 오버라이딩 해야한다.
	 */
	...
	public String getParameter(String name) {
		...
		case "loginMember.getMemPwd" :
			...
			break;
		default : encPwd = super.getParameter(name); // 이전 getParameter 유지
		}
...
//		해시함수 : 입력된 값을 여러 단계의 연산을 거처 일정한 길이의 무작위 값을 만들어내는 함수이다.
		
		try {
			
			md = MessageDigest.getInstance("SHA-512");
//			지정된 알고리즘(SHA-512)를 이용할 수 있는 MessageDigest 객체를 생성 후 얻어온다.
		}catch (NoSuchAlgorithmException e) {
			e.printStackTrace();
		}
//		암호화 전 전달 받은 문자열(입력 받은 비밀번호)를 바이트 배열 형식으로 변환해야한다.
