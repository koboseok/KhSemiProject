...
<body>
	...
	<div class="container my-5">
		...
		<h1>
			<span id="boardStyle"> 공지 사항 - 등록 </span>
		</h1>
		<hr>
		<div class="bg-white rounded shadow-sm container py-3"
			id="notice-area">
			<form method="POST" action="insert.do" role="form"
				onsubmit="return noticeValidate();">
				<div class="form-inline mb-2">
					<div class="input-group">
						<label class="input-group-addon mr-3">제목</label> <input
							type="text" class="form-control" id="noticeTitle"
							name="noticeTitle" size="70">
					</div>
				</div>
				...
				<div class="form-inline mb-2">
					<div class="input-group">
						...
						<h5 class="my-0">${loginMember.memName }</h5>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<textarea class="form-control" id="noticeContent"
						name="noticeContent" rows="15" style="resize: none;"></textarea>
				</div>
	...
	<script>
		(function printToday(){
			...
			var month = now.getMonth() + 1; // getMonth() : 0 ~ 11 중 하나가 반환 ( +1을 해야 1 ~ 12)
			var date = now.getDate(); // 4
			
			var str = year + "-" + (month < 10 ? "0" + month : month) + "-" + (date < 10 ? "0" + date : date);
			//						1 ~ 9 일때 앞에 0을 붙이고 아닐때는 그냥 출력 (10, 11, 12)
			$("#today").text(str);
