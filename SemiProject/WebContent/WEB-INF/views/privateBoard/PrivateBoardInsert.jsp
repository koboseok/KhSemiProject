...
<body>
	...
	<script>
		...
		$(function() {
			$("#fileArea").hide(); //#fileArea요소를 숨김
			$(".boardImg").on("click", function() { //이미지 영역이 클릭되었을 때
				//클릭한 이미지 영역 인덱스 얻어오기
				var index = $(".boardImg").index(this);
					//->클릭된 요소가 .boardImg중 몇 번째 인덱스인지 반환
					
				//console.log(index);
				//클릭된 영역 index에 맞는 input file태그 클릭 : 이미지 영역을 클릭할 때 input태그를 클릭한 것과 같은 효과
				$("#img" + index).click();
			});
		});
	  ...
	  function LoadImg(value, num) {
		  //value.files: 파일이 업로드 되어있으면 true
		  //value.files[0] : 여러 파일 중 첫 번째 파일이 업로드 되어있으면 true
		if(value.files && value.files[0]) { 
			var reader = new FileReader();
			//해당 요소에 업로드된 파일이 있을 경우
			// 자바스크립트 FileReader
			...
			//지정된 요소의 내용을 읽기 시작합니다.
			//blob완료되면 result속성 data:에 파일데이터를 나타내는 url이 포함됩니다.
			
			reader.onload = function(e) {
				//fileReader.onload
				//load이벤트의 핸들러.
				//이 이벤트는 읽기 동작이 성공적으로 완료되었을 때마다 발생합니다.
				
				//읽어들인 내용(이미지파일)을 화면에 출력
