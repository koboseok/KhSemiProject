...
function selectReplyList(){
   ...
   $.ajax({
      url : "${contextPath}/jointReply/selectList.do",
...
$("#addReply").on("click", function(){
   ...
   if(loginMemName == ""){
      ...
      if(replyContent.length ==0){
         ...
         $.ajax({
            url : "${contextPath}/jointReply/insertReply.do",
...
function updateReply(replyNo, el){
	...
	$.ajax({
 		url : "${contextPath}/jointReply/updateReply.do",
		type : "post",
...
function deleteReply(replyNo){
	...
	if(confirm("정말로 삭제하시겠습니까?")){
		var url = "${contextPath}/jointReply/deleteReply.do";
		...
		$.ajax({
			...
			success : function(result){
				if(result > 0){
					selectReplyList(parentBoardNo); // 댓글 목록을 다시 읽어옴.
